...
function submitAuthentication(username, password) {
	WL.Logger.info("In sumbitAuthentication");
	WL.Logger.info("User: "+ username );
	
	var input = {
		    method : 'get',
		    returnedContentType : 'json',
		    //path : 'presto/edge/api/rest/ms_te_incidents_agrupado/Invoke',
		    path : 'presto/edge/api/rest/cnoc_user_ldap/Invoke',
		    parameters: { 'x-presto-resultFormat': 'json', 'user': username },
		    headers: {"Accept":"application\/json",Authorization:encodeBaseAuth(username,password)} 
		};
	
	var status = 0;
	if (username == "cnoc" && password == "cnoc2016" ){
		status = 200;
	}else{
		status = 400;
	}
	//status = WL.Server.invokeHttp(input).getStatusLine().getStatusCode();
	//WL.Logger.info("Status:"+ status);
